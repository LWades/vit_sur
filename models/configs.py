...
def get_sur3_config():
    """Returns the surface code(L=3) configuration."""
    config = ml_collections.ConfigDict()
...
def get_sur5_config():
    """Returns the surface code(L=5) configuration."""
    config = ml_collections.ConfigDict()
    config.patches = ml_collections.ConfigDict({'size': (1, 1)})  # 这里要改, 16->1
    # config.patches = ml_collections.ConfigDict({'size': (1, 1)})  # 这里要改, 16->1
    config.hidden_size = 768
...
def get_sur11_config():
    ...
    # config.patches = ml_collections.ConfigDict({'size': (4, 4)})  # 这里要改, 16->3
...
def get_b16_config():
    ...
    config = ml_collections.ConfigDict()
    config.patches = ml_collections.ConfigDict({'size': (16, 16)})  # 这里要改
    config.hidden_size = 768
    config.transformer = ml_collections.ConfigDict()
    config.transformer.mlp_dim = 3072
    config.transformer.num_heads = 12
